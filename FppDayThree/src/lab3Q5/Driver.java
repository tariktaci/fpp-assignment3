...
import lab3Q5.more.StoreDirectory;
...
public class Driver {
	...
	void displayNumberOfEmployees() {
		...
		int numberemployee = directory.getNumberOfBookstoreEmployees();
		System.out.println(
				"Number of employees is: " + numberemployee + " (Look for other console messages.)" + n + CONTINUE);

	}

	void addEmployee(String employeeId) {
		// implement-- redo the implementation
		boolean value = directory.addNewEmployee(employeeId);
		if (value)
	...
	void displayNumberOfCDsInBookstore() {
		// implement-- redo the implementation
		int value = directory.getNumberOfBookstoreEmployees();
		System.out.println("Number of CD's is " + value + " (Look for console messages.)" + n + CONTINUE);
